...
export class ArticoliService {
  ...
  */
  ...
  getAricoliByDesc(descrizione: string) {
    ...
    return this.http.get<Iarticoli[]>(url).pipe(
      map((dati: Iarticoli[]) => dati.forEach(element => {
        element.idStatoArt = (element.idStatoArt == '1') ? "Attivo" : "Sospeso";
        return dati;
      })
      )
    );
  ...
  /* da un errore da risolvere piu avanti
